...
procinit(void)
{
  ...
  for(p = proc; p < &proc[NPROC]; p++) {
      ...
      /*
       for lab 3-2, kernel stack will be mapped to proc's kernel page table 
      */
      // char *pa = kalloc();
      // if(pa == 0)
      //   panic("kalloc");
...
scheduler(void)
{
  ...
  for(;;){
    ...
    if(found == 0){
      // Load kernel pagetable (unique one) into satp
      kvminithart();
    }
#if !defined (LAB_FS)
    if(found == 0) {
