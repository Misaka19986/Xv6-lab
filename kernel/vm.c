...
// Recursively print pte (given depth)
void 
vmprint_depth(pagetable_t pagetable, int depth)
{
  ...
  for(int i = 0;i < 512;i++){
    pte_t pte = pagetable[i];
    if(pte & PTE_V){
      printf("%s%d: pte %p pa %p\n", indent[depth - 1], i, pte, PTE2PA(pte));
    }
    if((pte & PTE_V) && (pte & (PTE_R|PTE_X)) == 0){
      // has child
      uint64 child = PTE2PA(pte);
...
vmprint(pagetable_t pagetable)
{
  ...
  vmprint_depth(pagetable, 1);
}

// mapping to given pagetable
void 
...
// mapping a kernel page table for user proc
pagetable_t
...
// so that kernel can access user pa directly
int
u2kvmcopy(pagetable_t upagetable, pagetable_t kpagetable, uint64 begin, uint64 end)
{
  ...
  for(i = begin_page;i < end;i += PGSIZE){
    if((pte = walk(upagetable, i, 0)) == 0) panic("u2kvmcopy: pte should exist");
    if((*pte & PTE_V) == 0) panic("u2kvmcopy: page not present");
    pa = PTE2PA(*pte);
    flags = PTE_FLAGS(*pte) & (~PTE_U); // clear PTE_U
